...
import { Space, Table, Tag, message, Button, Row, Col, Popconfirm, Image } from "antd";
...
import { deleteBlog, getBlog } from "../../../service/blogs";
...
const Blog = () => {
  ...
  const onDeleteRow = async (Row) => {
    ...
    try {
      await deleteBlog(Row?._id).then(({ data }) => {
        console.log(data);
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      render:(_,item)=>{
        return(<Image width={50} height={50} src={item.thumbnail}/>)
      }
    },
    {
      title: "Blog Title",
  ...
  const onChange = (val) => {};
  const onAddRow = (data) => {
    router.push({ pathname: "/blog/blogForm", query: { ...data } });
  };
